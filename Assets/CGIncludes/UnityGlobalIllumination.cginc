...
//<MLS_GLOBAL_ILLUMINATION_INCLUDE_SECTION>
...
inline UnityGI UnityGI_Base(UnityGIInput data, half occlusion, half3 normalWorld)
{
    ...
    o_gi.light.color *= data.atten;
        ...
        #if defined(_MLS_ENABLE_LIGHTMAPS_BLENDING)
            half4 bakedColorTex = BlendTwoTextures(0, data.lightmapUV.xy);
        #else
            half4 bakedColorTex = UNITY_SAMPLE_TEX2D(unity_Lightmap, data.lightmapUV.xy);
        #endif

        //<FTRACEV1.0>
        #if defined(USEFTRACE) && !defined(_MLS_ENABLE_LIGHTMAPS_BLENDING)
            half3 bakedColor = ftLightmapBicubic(data.lightmapUV.xy);
        #else
        ...
        //</FTRACEV1.0>
            ...
            // Magic Lightmap Switcher
            #if defined(_MLS_ENABLE_LIGHTMAPS_BLENDING)
                fixed4 bakedDirTex = BlendTwoTextures(1, data.lightmapUV.xy);
            #else
                fixed4 bakedDirTex = UNITY_SAMPLE_TEX2D_SAMPLER (unity_LightmapInd, unity_Lightmap, data.lightmapUV.xy);
            #endif
            //</MLS_GLOBAL_ILLUMINATION_UNITY_GI_BASE_ADDITIONS>
...
inline half3 UnityGI_IndirectSpecular(UnityGIInput data, half occlusion, Unity_GlossyEnvironmentData glossIn)
{
    half3 specular;
        ...
        #ifdef _MLS_ENABLE_REFLECTIONS_BLENDING
